...
macro_rules! read_2 {
    ...
    ($inst:path, $buf:ident,$reg:expr) => {
        unimplemented!()
    };

    ($inst:path, $buf:ident,$reg:expr, $reg2:expr) => {
        unimplemented!()
    };
}
...
impl Memory {
    ...
    fn next_instruction(&mut self) -> Result<Instruction, String> {
        ...
        use std::ops::Index;
        let code =
            OpCode::from_u8(*self.buf.index(self.pos)).ok_or("out of range".to_string())?;
        let result = match code {
            ...
            | OpCode::NOP_8
            | OpCode::NOP_9
            | OpCode::NOP_10 => Ok(Instruction::NOP),
            ...
            // LXI
            OpCode::LXI_B_D => read_3!(Instruction::LXI, buf, B, D),
            OpCode::LXI_D_D => read_3!(Instruction::LXI, buf, D, D),
            OpCode::LXI_H_D => read_3!(Instruction::LXI, buf, H, D),
            OpCode::LXI_SP_D => read_3!(Instruction::LXI, buf, SP, D),

            OpCode::JNC => read_3!(Instruction::JNC, buf),

            OpCode::JMP => read_3!(Instruction::JMP, buf),

            OpCode::STAX_B => read_3!(Instruction::STAX, buf, B),
            OpCode::STAX_D => read_3!(Instruction::STAX, buf, D),
            OpCode::STA => read_3!(Instruction::STA, buf),
            ...
            // Mvi
            OpCode::MVI_A_D => read_2!(Instruction::MVI, buf, A, D),
            OpCode::MVI_B_D => read_2!(Instruction::MVI, buf, B, D),
            OpCode::MVI_C_D => read_2!(Instruction::MVI, buf, C, D),
            OpCode::MVI_D_D => read_2!(Instruction::MVI, buf, D, D),
            OpCode::MVI_E_D => read_2!(Instruction::MVI, buf, E, D),
            OpCode::MVI_H_D => read_2!(Instruction::MVI, buf, H, D),
            OpCode::MVI_L_D => read_2!(Instruction::MVI, buf, L, D),
            OpCode::MVI_M_D => read_2!(Instruction::MVI, buf, M, D),
            ...
            OpCode::SUI => read_2!(Instruction::SUI, buf),

            OpCode::XTHL => read_1!(Instruction::XTHL),
            OpCode::PCHL => read_1!(Instruction::PCHL),

            OpCode::CALL => read_3!(Instruction::CALL, buf),
            OpCode::IN => read_2!(Instruction::IN, buf),
            OpCode::ORI => read_2!(Instruction::ORI, buf),
            OpCode::ADI => read_2!(Instruction::ADI, buf),

            OpCode::JC => read_3!(Instruction::JC, buf),
            OpCode::LDA => read_3!(Instruction::LDA, buf),
            OpCode::JNZ => read_3!(Instruction::JNZ, buf),
            ...
            OpCode::JZ => read_3!(Instruction::JZ, buf),
            OpCode::CNZ => read_3!(Instruction::CNZ, buf),
            OpCode::LHLD => read_3!(Instruction::LHLD, buf),
            ...
            OpCode::CPI => read_2!(Instruction::CPI, buf),
            OpCode::CNC => read_3!(Instruction::CNC, buf),
            OpCode::OUT => read_2!(Instruction::OUT, buf),
            ...
            OpCode::SHLD => read_3!(Instruction::SHLD, buf),
            ...
            OpCode::ANI => read_2!(Instruction::ANI, buf),
            OpCode::JPO => read_3!(Instruction::JPO, buf),
            OpCode::CM => read_3!(Instruction::CM, buf),
            OpCode::CPE => read_3!(Instruction::CPE, buf),
            ...
            OpCode::JM => read_3!(Instruction::JM, buf),

            OpCode::SBI => read_2!(Instruction::SBI, buf),
            OpCode::XRI => read_2!(Instruction::XRI, buf),
...
//                 match read_code(code, &mut self.buf) {
//                     Ok((inst, i)) => {
//                         self.pc += i;
